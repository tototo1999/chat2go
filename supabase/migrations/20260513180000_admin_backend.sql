-- 管理员后台基础设施
-- email 白名单 → is_admin() + 一组 SECURITY DEFINER RPC 让 admin 绕过 RLS
-- 用 $body$ 命名 dollar quote（避免 SQL Editor 的 $$ 解析坑）

-- 1) profiles 加 banned_at
...
begin
  ...
  select
    ...
    (select count(*) from profiles where role='expert')::bigint,
    (select count(*) from profiles where banned_at is not null)::bigint,
    (select count(*) from profiles where created_at >= date_trunc('day', now()))::bigint,
    (select count(*) from rooms)::bigint,
    (select count(*) from messages)::bigint,
    (select count(*) from messages where created_at >= date_trunc('day', now()))::bigint,
    (select count(*) from model_usage where created_at >= date_trunc('day', now()))::bigint,
    coalesce((select sum(cost_usd) from model_usage), 0)::numeric,
    coalesce((select sum(user_charge_cny) from model_usage), 0)::numeric,
    coalesce((select sum(cost_usd) from model_usage where created_at >= date_trunc('day', now())), 0)::numeric,
    coalesce((select sum(user_charge_cny) from model_usage where created_at >= date_trunc('day', now())), 0)::numeric
  ;
end;
$body$;
...
begin
  ...
  select
    ...
    (select count(*) from messages m
       where m.user_id = p.user_id and m.role <> 'ai')::bigint
  from profiles p
...
begin
  ...
  select
    ...
    u.email::text,
    p.display_name,
    (select count(*) from messages m where m.room_id = r.id)::bigint,
    coalesce((select sum(cost_usd) from model_usage mu where mu.room_id = r.id), 0)::numeric,
    coalesce((select sum(user_charge_cny) from model_usage mu where mu.room_id = r.id), 0)::numeric,
    (select count(*) from room_members rm where rm.room_id = r.id)::bigint,
    r.created_at
  from rooms r
...
begin
  ...
  select
    ...
    p.display_name,
    l.last_seen_at,
    (l.last_seen_at is not null and l.last_seen_at > now() - interval '5 minutes'),
    coalesce(l.msgs_24h, 0)::bigint,
