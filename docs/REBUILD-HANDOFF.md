...
>
> **2026-05-16 draft v0.1**:基于本机已跑通的 Hermes 集成实现写成,代码细节都来自 `~/.hermes/hermes-agent/gateway/platforms/chat2go.py`(本地参考实现)+ `chat2go-agent/` 旧 Python 包(可去看历史踩坑)。
...
### 5.4 当前(2026-05-16)状态:**断了**

- Hermes 接管后,memories 表 0 新增(实测 1 小时内)
- Hermes 用自己的 `~/.hermes/memories/MEMORY.md` 文件式 memory
- Hermes memory 和 chat2go memories 表之间**没有桥接**

### 5.5 你重建时的 3 个选项

| 选项 | 说明 | 推荐度 |
|---|---|---|
| **(a) 复刻 chat2go-agent 的 sync_memory** | 在你的 backend 里实现:大咖消息后,LLM 提取 fact,JSON,写 `memories` 表 + prefetch 注入 prompt | ★★★ 跟前端契约最对,产品差异化能延续 |
...
**推荐 (a)**。chat2go-agent `chat2go_agent/memory.py`(165 行,git tag `archived-v0.2-pre-hermes` 可参考)有现成的 sync_memory + prefetch_memory 实现,直接照搬逻辑。
...
- **chat2go 仓库 HANDOFF.md** — 完整技术蓝图(本文件的母版)
- **chat2go 仓库 docs/AGENT_DESIGN.md** — Phase 2 Skills / RAG 设计文档
- **chat2go 仓库 docs/CHANGELOG-2026-05-15.md** — 昨天 47 commit 摘要,memory 6 层 bug 链都在
- **chat2go-agent(归档版,git tag `archived-v0.2-pre-hermes`)** — chat2go_agent/memory.py 是 sync_memory 完整实现
- **本机 `~/.hermes/hermes-agent/gateway/platforms/chat2go.py`** — 当前跑通的 Hermes 集成实现,533 行,含 bridge_pong / model_usage / inbound dispatch / send 全套
- **Supabase Dashboard** → SQL Editor 可以查所有表的当前状态
