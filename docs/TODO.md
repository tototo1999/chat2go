...
> 造句/默写是把"认得"变成"会用/会写"的关键一环,当前是闭环里最薄的一段。

- [ ] **造句游戏优化** — 现状:出题只从通用 `SENTENCE_BANK`(100 句中译英模板,`buildSentencePrompt` 用 `idx % length` 取),AI 评分走 `glossary-grade`,有 AB 对比 / 换一题 / 造句历史。痛点 + 优化方向:
  - **题目跟当天学的脱节** — worker 已产出 `practice_patterns`(5-10 真实句式)但只进了 summary 卡片,没进造句游戏。→ 把 lesson 的 practice_patterns 作为优先题源,通用模板兜底。
  - **结果不回写艾宾浩斯** — 造句评分(S/A/B/C/D)目前不影响 `review_stage`。→ 造对(≥A)推进阶段,造错降级/重排 due,让造句真正驱动复习曲线。
  - **难度不分级** — 不按 review_stage / 掌握度出题。→ 新词给简单模板,熟词给开放式/听写式。
  - **可加语音题干** — TTS 读中文题干 → 听着造句(更接近真实开口)。
- [ ] **单词默写(新功能)** — 现状:**完全没有**,单词表只有 🎮 拆音节练习 + 造句,缺"看/听 → 默写英文"的拼写主动回忆。要做:
  - **中→英 默写**:显示中文 + 罗马注音提示(可隐藏),用户拼英文 → 逐字母校验 + 高亮错处。
  - **听写默写**:TTS 读英文 → 用户盲拼 → 校验(最接近真实拼写能力)。
  - **驱动艾宾浩斯**:默写对推进 `review_stage`,错的自动进"错题本"重排 due — 跟造句共用同一套 stage 推进逻辑。
  - **错题本**:默写/造句错的词自动汇总成一个动态 tab,优先复习。
  - 入口:卡片上加 ✍️ 默写按钮(跟 🎮 并列),或复习 session 里穿插默写题。
- [ ] **「我已记住」词库(毕业归档)** — 现状:词到 `review_stage 6` 只在原地显示「✓ 掌握」徽章,没毕业去处,熟词跟生词混在同一 tab 越堆越多。要做:
  - 单独一个 **「✅ 已记住」词库 tab**,词掌握(stage 6 或手动标记)后从活跃复习里**毕业**进这里,不再占用日常复习。
  - 已记住词库支持回看 / 抽查(防遗忘随机复测,答错可"打回"重新进复习曲线)。
  - 跨所有 lesson / 分类汇总,是用户"学习成果"的总账本(可显示总掌握数,成就感)。
- [ ] **句型库(句式持久沉淀)** — 现状:worker 已产出 `practice_patterns`(5-10 真实句式),但**只渲染进 summary 卡片**(`## 📝 句式练习` markdown),一次性、不沉淀、不能复用。要做:
  - 跟单词表平级的 **「句型库」**:每节课的 practice_patterns 自动入库(去重),成为可滚动积累的句式资产。
...
- [ ] **专项练习 · 不规则动词过去式/过去分词** — 现状:无。要做一个独立**主题练习库**:
  - 三态默写:给原形 → 默写过去式 + 过去分词(go→went→gone / take→took→taken),逐个校验。
  - 分组归类:按变化模式分主题(A-B-B 如 sit/sat/sat、A-B-C 如 sing/sang/sung、A-A-A 如 cut/cut/cut、不规则特例),一组一组练。
  - 跟艾宾浩斯打通:错的三态自动重排 due,进错题本。
  - 内置一份高频不规则动词表(~100 个),用户也可把课上遇到的加进来。
- [ ] **专项练习 · -ed 三种发音分类库** — 现状:无。规则动词 -ed 结尾的 3 种读音是经典发音难点,做一个**发音分类练习库**:
  - 三类:**/t/**(清辅音后:walked / watched / kissed)、**/d/**(浊辅音/元音后:played / called / opened)、**/ɪd/**(t/d 后:wanted / needed / decided)。
  - 玩法:给一个 -ed 词 → 用户判断它属于哪一类发音 → TTS 朗读验证;或反向"听音归类"。
  - 跟现有 🎮 拆音节/标重音练习同一套路(发音主动回忆),共用 stage 推进 + 错题本。
  - 内置规则词库 + 课上遇到的规则动词自动可入库。
