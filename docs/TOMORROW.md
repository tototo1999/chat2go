...
### 1. Agent 部署形态
- **A**：每个专家自己电脑跑 Agent + bridge（专家本地资源 / 隐私好 / 门槛高）
- **B**：平台云端跑 Agent，专家无需安装（无门槛 / 平台承担成本）
- **C**：混合——免费专家用云端版本，付费专家可选本地自托管（更强能力 / 数据自主）
...
2. **数据所有权清晰**：每个 skill / memory / KB 必须知道归谁所有（专家 / 房间 / 平台）
3. **可调教 > 全自动**：宁可专家手动改 prompt，也不要做猜不准的"智能"
