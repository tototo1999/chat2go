...
**Chat2GO.Ai Agent 是一个 marketplace 专用 AI agent，不追求 Hermes 的「啥都能干」，而追求「大咖的行业能力 → AI → 小白」这一条链路最短最稳。**

核心特性（继承 Hermes 但重构）：

| Hermes 能力 | Chat2GO.Ai 落地 | 改动点 |
|------------|-------------|-------|
| 自学习完善 md | 大咖纠正 → 自动沉淀到 skill md | 增加「双 AI」链路（主答 + 学习） |
| 多模态工具箱 | 文件 / 图片 / PDF / OCR / Web / KB | 工具列表精简，行业向 |
| 快速切换大模型 | Router by task complexity + room.model | 加成本/性能维度的自动路由 |
| 知识库 | 大咖上传 → pgvector → RAG | Supabase 原生，无独立服务 |
...
└──────────────────────────────────────────────────────────────────┘
              ...
              │                                │
┌──────────────────────────────────────────────────────────────────┐
│            Chat2GO.Ai Agent Bridge (本地 / 云端 都可)                  │
│  ┌────────────┐ ┌──────────┐ ┌─────────┐ ┌────────────────────┐ │
...
┌──────────────────────────────────────────────────────────────────┐
│                  Learner（学习子 agent）                           │
│   监控大咖纠正 → LLM 总结 → 写回 skill.lessons.md                   │
└──────────────────────────────────────────────────────────────────┘
```
...
rooms (
  ...
  system_prompt text default ''          -- 房间级大咖指令
)
...
create table skills (
  ...
  is_public     boolean default false,   -- 是否对其他大咖可见
...
### 3.1 模块一：大咖纠正自动沉淀（Lessons）★ 优先级最高
...
背景：刚才小白问 X，AI 答了 Y
现在大咖说：Z
请提取「大咖想纠正 AI 的规则」，1-2 行简洁描述。
如果大咖只是聊天/澄清，不是纠正，输出 NULL。
```
...
| 整个 skill | `lessons` 表 + skill 重新生成时合并 | 大咖点「保存到 skill」按钮（未来 UI）|

#### 3.1.3 用户体验

大咖发了纠正消息后，几秒后看到一条小提示：
...
这是 Chat2GO.Ai 商业模式的核心：**大咖的私有知识 = 他的资产**。
...
**Phase 1 出来的产品演示价值：** 大咖给小白做合同 → 小白要 PDF → 大咖说"违约金改 10%" → 下次自动按新规则。
...
**Phase 2 价值：** 大咖上传整套行业资料 → AI 用大咖知识精准回答 → 这是 Chat2GO.Ai 的真实差异化。
...
| 维度 | Hermes | Chat2GO.Ai Agent | 为何不同 |
|------|--------|---------------|---------|
| 部署 | 每用户本地 | 云端 bridge | 小白不会装 CLI |
| 上下文 | 本地 sessions | Supabase rooms | 多人共享 / 跨设备 |
| 学习 | 用户改 md | **大咖纠正自动学** | marketplace 自带反馈循环 |
| 知识 | 各种 search skill | **大咖私有 KB** | 知识 = 大咖资产 |
...
| 风险 | 应对 |
|------|------|
| 大咖自学习写错规则 | UI 上让大咖审核 lessons，可一键回滚 |
| 知识库被大咖"窃取"（爬虫导出）| 行级 RLS + 查询频率限制 + 不返回 raw chunk，只通过 AI 转述 |
| API 成本失控 | model_usage 表 + 每房间月度上限 |
| Realtime 断线 | bridge 已有轮询兜底 |
| 单大咖学习污染 skill | lessons 默认仅房间级，大咖手动晋升才进 skill |

**关键决策（待定）：**
1. **Bridge 部署模型**：每个大咖本地跑？还是平台统一云端？（影响隐私/成本/稳定性）
2. **API Key 归属**：大咖自带（更便宜，大咖承担成本）？还是平台统一（更可控，平台向小白收费）？
